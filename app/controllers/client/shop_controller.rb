  ...
  def buy
    if current_client_user
      if @offer.one_time?
        ...
        if Order.exists?(user: current_client_user, offer: @offer, state: [:pending, :submitted, :paid]) ||
          Order.where(user: current_client_user, offer: @offer).where('created_at >= ?', Time.now.beginning_of_month).exists?
          flash[:alert] = "You can only purchase this monthly offer once per month."
          redirect_to client_shop_index_path and return
        end
      elsif @offer.weekly?
        if Order.exists?(user: current_client_user, offer: @offer, state: [:pending, :submitted, :paid]) ||
          Order.where(user: current_client_user, offer: @offer).where('created_at >= ?', Time.now.beginning_of_week).exists?
          flash[:alert] = "You can only purchase this weekly offer once per week."
          redirect_to client_shop_index_path and return
        end
      elsif @offer.daily?
        if Order.exists?(user: current_client_user, offer: @offer, state: [:pending, :submitted, :paid]) ||
          Order.where(user: current_client_user, offer: @offer).where('created_at >= ?', Time.now.beginning_of_day).exists?
